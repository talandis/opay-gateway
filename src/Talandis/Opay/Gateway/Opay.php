...
class Opay implements CoreInterface, WebServiceInterface
{
        ...
        if (!empty($this->merchantRsaPrivateKey) && !empty($this->opayCertificate)) {
            if (function_exists('openssl_pkey_get_public')) {
                ...
                if (!empty($this->signaturePassword)) {
                    ...
                    throw new Exception('OpenSSL is not available in your server. To use RSA signature type (which is set when using setMerchantRsaPrivateKey() and setOpayCertificate()) install the OpenSSL PHP module. Otherwise use password signature (which is set when using setSignaturePassword()).', Exception::SIGNATURE_OPEN_SSL_NOT_FOUND);
                }
            }
        } else {
            if (!empty($this->signaturePassword)) {
                ...
                throw new Exception('Signature parameters are not set. Use functions setMerchantRsaPrivateKey() and setOpayCertificate() to set parameters for RSA signature type, or setSignaturePassword() for password signature type.', Exception::SIGNATURE_PARAMETERS_ARE_NOT_SET);
            }
        }
    }
            ...
            if (openssl_sign($stringToBeSigned, $signature, $pkeyid) === true) {
                if ($toBase64Encode == true) {
                    ...
                    if (($signature = base64_encode($signature)) !== false) {
                        ...
                        throw new Exception('Could not encode to base64 after signing using a private key.', Exception::SIGNING_USING_PRIVATE_KEY_BASE_64_ERROR);
                    }
                } else {
                    return $signature;
                }
            } else {
                throw new Exception('Error occurred when signing using private key', Exception::SIGNING_USING_PRIVATE_KEY_ERROR);
            }
        } else {
            throw new Exception('Error reading private key', Exception::SIGNING_USING_PRIVATE_KEY_READING_ERROR);
        }
    }
    ...
    public function convertEncodedStringToArrayOfParameters($encodedString)
    {
        ...
        if (($data = base64_decode($data)) !== false) {
            ...
            throw new Exception('Base64 decoding error when converting encoded request from gateway.', Exception::GATEWAY_REQUEST_BASE64_DECODE_ERROR);
        }
    }
    ...
    public function verifySignature($parametersArray)
    {
        ...
        if (!empty($rsaSignature) && !empty($this->opayCertificate)) {
            ...
            if (!empty($passwordSignature) && !empty($this->signaturePassword)) {
                ...
                throw new Exception('Could not verify a signature. Signature parameters are not set properly. Use functions setMerchantRsaPrivateKey() and setOpayCertificate() to set parameters for RSA signature type, or setSignaturePassword() for password signature type. ', Exception::SIGNATURE_PARAMETERS_ARE_NOT_SET);
            }
        }
    }
        ...
        if ($pubkeyid !== false) {
            ...
            if ($ok === 1) {
                ...
                if ($ok === 0) {
                    ...
                    throw new Exception('Error reading certificate or extracting a public key from it', Exception::SIGNATURE_VERIFICATION_USING_CERTIFICATE_ERROR);
                }
            }
        } else {
            throw new Exception('Error reading certificate or extracting a public key from it', Exception::SIGNATURE_VERIFICATION_USING_CERTIFICATE_READING_ERROR);
        }
    }
        ...
        if ($data !== false) {
            ...
            if ($decodeJson) {
                if (version_compare(PHP_VERSION, '5.2.0', '<')) {
                    throw new Exception('Cannot decode JSON. Your PHP version (' . PHP_VERSION . ') does not have json_decode function. This function is included starting from PHP version 5.2.0. You may pass FALSE to $decodeJson parameter when calling OpayGateway::webServiceRequest method and decode JSON by yourself.', Exception::JSON_DECODING_ERROR);
                }
                ...
                if (version_compare(PHP_VERSION, '5.3.0', '>=')) {
                    ...
                    if ($jsonLastError != JSON_ERROR_NONE) {
                        throw new Exception('Could not decode JSON. json_decode() error code is ' . $jsonLastError, Exception::JSON_DECODING_ERROR);
                    }
                } else {
                    if ($data === NULL) {
                        throw new Exception('Wrong JSON format or the encoded data is deeper than the recursion limit.', Exception::JSON_DECODING_ERROR);
                    }
                }

                if (!is_array($data)) {
                    throw new Exception('Didn\'t get an array after decoding the JSON returned by the web service', Exception::WRONG_JSON_FORMAT);
                }
            }
            return $data;
        } else {
            throw new Exception('Could not connect to server.', Exception::COMMUNICATION_WITH_SERVER_ERROR);
        }
    ...
    public function sendRequest($url, $httpMethod, $parametersArray, $keepAlive = false, $optionalHeaders = null, $timeout = 3)
    {
        ...
        if (!array_key_exists('scheme', $parsedUrlArr)) {
            throw new Exception('URL must contain a name of a protocol e.g. http:// or https://', Exception::COMMUNICATION_WITH_SERVER_ERROR);
        }

        if (strtolower($parsedUrlArr['scheme']) == 'https') {
            $w = stream_get_wrappers();
            if (!extension_loaded('openssl') || !in_array('https', $w)) {
                throw new Exception('php_openssl extension have to be enabled and allow_url_fopen must be set On', Exception::COMMUNICATION_WITH_SERVER_ERROR);
            }
        }
        ...
        if ($httpMethod == 'GET') {
            ...
            $url .= '?' . http_build_query($parametersArray);
        } else {
            $params['http']['method'] = 'POST';
            $params['http']['content'] = http_build_query($parametersArray);
        }
    ...
    protected function iso369_3ToIso369_1($languageCode)
    {
        $arr = array(
            ...
            'bul' => 'bg', 'bis' => 'bi', 'bam' => 'bm', 'ben' => 'bn', 'bod' => 'bo',  'bre' => 'br',
            'bos' => 'bs', 'cat' => 'ca', 'che' => 'ce', 'cha' => 'ch', 'cos' => 'co', 'cre' => 'cr', 'ces' => 'cs',
             'chu' => 'cu', 'chv' => 'cv', 'cym' => 'cy', 'dan' => 'da', 'deu' => 'de',
             'div' => 'dv', 'dzo' => 'dz', 'ewe' => 'ee', 'ell' => 'el', 'eng' => 'en',
            'epo' => 'eo', 'spa' => 'es', 'est' => 'et', 'eus' => 'eu', 'fas' => 'fa',
            'ful' => 'ff', 'fin' => 'fi', 'fij' => 'fj', 'fao' => 'fo', 'fra' => 'fr', 'fry' => 'fy',
            'gle' => 'ga', 'gla' => 'gd', 'glg' => 'gl', 'grn' => 'gn', 'guj' => 'gu', 'glv' => 'gv', 'hau' => 'ha',
            'heb' => 'he', 'hin' => 'hi', 'hmo' => 'ho', 'hrv' => 'hr', 'hat' => 'ht', 'hun' => 'hu', 'hye' => 'hy',
             'her' => 'hz', 'ina' => 'ia', 'ind' => 'id', 'ile' => 'ie', 'ibo' => 'ig', 'iii' => 'ii',
            'ipk' => 'ik', 'ido' => 'io', 'isl' => 'is', 'ita' => 'it', 'iku' => 'iu', 'jpn' => 'ja',
            'jav' => 'jv', 'kat' => 'ka',  'kon' => 'kg', 'kik' => 'ki', 'kua' => 'kj', 'kaz' => 'kk',
            ...
            'mri' => 'mi',  'mkd' => 'mk', 'mal' => 'ml', 'mon' => 'mn', 'mar' => 'mr',
            'msa' => 'ms', 'mlt' => 'mt', 'mya' => 'my', 'nau' => 'na', 'nob' => 'nb',
            'nde' => 'nd', 'nep' => 'ne', 'ndo' => 'ng', 'nld' => 'nl', 'nno' => 'nn', 'nor' => 'no',
            'nbl' => 'nr', 'nav' => 'nv', 'nya' => 'ny', 'oci' => 'oc', 'oji' => 'oj', 'orm' => 'om', 'ori' => 'or',
            'oss' => 'os', 'pan' => 'pa', 'pli' => 'pi', 'pol' => 'pl', 'pus' => 'ps', 'por' => 'pt', 'que' => 'qu',
            'roh' => 'rm', 'run' => 'rn', 'ron' => 'ro', 'rus' => 'ru', 'kin' => 'rw', 'san' => 'sa',
            'srd' => 'sc', 'snd' => 'sd', 'sme' => 'se', 'sag' => 'sg', 'sin' => 'si', 'slk' => 'sk',
            'slv' => 'sl', 'smo' => 'sm', 'sna' => 'sn', 'som' => 'so', 'sqi' => 'sq', 'srp' => 'sr',
